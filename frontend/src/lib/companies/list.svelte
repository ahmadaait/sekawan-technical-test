...
<div class="row">
	<div class="col-md-6">
		<button
			...
			on:click={() => goto('/admin-area/companies/add')}><i class="bi bi-plus-lg" /></button
		>
	</div>
	<div class="col-md-6">
		<input
		...
		<tbody>
			{#each items as item, i}
				<tr>
					...
					<td>{item.name}</td>
					<td>{item.address}</td>
					<td class="text-center">{item.phone ? item.phone : '-'}</td>
					<td class="text-center">{item.email ? item.email : '-'}</td>
					<td class="text-center">
						<button
							on:click={() => goto(`companies/edit?id=${item.id}`)}
							class="btn btn-primary btn-sm"
						...
						<button
							on:click={() => {
								...
								$modalSelectedContext = 'companies';
